...
class CRSCatalog:
    ...
    def validate(self, crs_code: str) -> None:
        if crs_code == "unset":
            return
        if crs_code not in self.entries:
            raise ValueError(f"Unknown CRS code: {crs_code}")
...
class StratUnit:
    ...
    def from_row(cls, row: Mapping[str, str]) -> StratUnit:
        ...
        return cls(
            ...
            parent_unit_id=parse_int("parent_unit_id"),
            ...
            rank_id=parse_int("rank_id"),
            start_age_ma=parse_float("start_age_ma"),
            end_age_ma=parse_float("end_age_ma"),
            red=parse_int("red"),
            green=parse_int("green"),
            blue=parse_int("blue"),
            standard_sort=parse_int("standard_sort"),
        )
        ...
        for point in self.points:
            for attr in required:
                if getattr(point, attr) is None:
                    raise ValueError(
                        f"Deviation point is missing required field '{attr}' "
                        f"for {self.reference.value}+{self.mode.value}"
                    )

        depth_values = [getattr(point, depth_field) for point in self.points]
