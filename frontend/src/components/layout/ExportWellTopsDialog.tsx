...
interface ExportWellTopsDialogProps {
...
async function postTopsExport(payload: Record<string, unknown>): Promise<Response> {
  return fetch('/api/export/wells/tops', {
    method: 'POST',
...
export function ExportWellTopsDialog({
  initialScope,
  activeWellId,
  wells,
  onClose,
}: ExportWellTopsDialogProps) {
  ...
  const exportableWells = useMemo(
    () => wells.filter((well) => well.active_top_set_id !== null && well.formations.some((top) => top.depth_md !== null)),
    [wells],
  )
  const title = scope === 'current' ? 'Export current well tops CSV' : 'Export all wells tops CSV'
  const canSubmit = scope === 'all'
    ? exportableWells.length > 0
    : currentWell !== null && currentWell.active_top_set_id !== null && currentWell.formations.some((top) => top.depth_md !== null)
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (scope === 'current') {
        const response = await postTopsExport({
          scope: 'current',
        ...
        if (outputDir) {
          ...
          await downloadResponse(response, `${currentWell?.well_name ?? 'well'}_tops.csv`)
        }
        return
      }
      ...
      if (packaging === 'one_file_per_well' && !exportToZip && !outputDir) {
        for (const well of exportableWells) {
          ...
          const response = await postTopsExport({ scope: 'current', well_id: well.well_id })
          // eslint-disable-next-line no-await-in-loop
          await downloadResponse(response, `${well.well_name}_tops.csv`)
        }
        return
      }
      ...
      if (outputDir) {
        ...
        await downloadResponse(response, exportToZip ? 'tops.zip' : 'tops.csv')
      }
    } catch (cause) {
  ...
  return (
    <form className="project-dialog" onSubmit={handleSubmit}>
      ...
      <div className="project-dialog__body">
        ...
        {!canSubmit ? <p className="project-dialog__error">No active TopSet picks are available for export.</p> : null}
