...
export function useDataManagerController() {
  ...
  return {
    ...
    setFormationUpdate: (formationId: string, patch: { name?: string; age_ma?: number; hiatus_duration_ma?: number; kind?: string; color?: string; water_depth_m?: number; eroded_thickness_m?: number; sea_level_m_override?: number | null }) => void updateFormation(formationId, patch),
    sidebarTopRatio,
    sidebarWidth,
    stratCharts,
